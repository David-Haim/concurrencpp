...
size_t default_max_cpu_workers() noexcept {
  return static_cast<size_t>(thread::hardware_concurrency() *
                             consts::k_cpu_threadpool_worker_count_factor);
}

size_t default_max_background_workers() noexcept {
  return static_cast<size_t>(
      thread::hardware_concurrency() *
      consts::k_background_threadpool_worker_count_factor);
}

constexpr static auto k_default_max_worker_wait_time =
...
void executor_collection::register_executor(
    std::shared_ptr<executor> executor) {
  std::unique_lock<decltype(m_lock)> lock(m_lock);
  assert(std::find(m_executors.begin(), m_executors.end(), executor) ==
         m_executors.end());
...
runtime_options::runtime_options() noexcept :
    ...
    max_background_thread_waiting_time(
        details::k_default_max_worker_wait_time) {}
...
runtime::runtime(const runtime_options& options) {
  ...
  m_thread_pool_executor =
      std::make_shared<::concurrencpp::thread_pool_executor>(
          details::consts::k_thread_pool_executor_name, options.max_cpu_threads,
...
std::shared_ptr<concurrencpp::timer_queue> runtime::timer_queue()
    const noexcept {
  return m_timer_queue;
}

std::shared_ptr<concurrencpp::inline_executor> runtime::inline_executor()
    const noexcept {
  return m_inline_executor;
}

std::shared_ptr<concurrencpp::thread_pool_executor>
runtime::thread_pool_executor() const noexcept {
  return m_thread_pool_executor;
}

std::shared_ptr<concurrencpp::thread_pool_executor>
runtime::background_executor() const noexcept {
  return m_background_executor;
}

std::shared_ptr<concurrencpp::thread_executor> runtime::thread_executor()
    const noexcept {
