...
bool wait_context::wait_for(size_t milliseconds) noexcept {
  std::unique_lock<std::mutex> lock(m_lock);
  return m_condition.wait_for(lock, std::chrono::milliseconds(milliseconds),
                              [this] {
                                return m_ready;
                              });
}
...
void result_core_base::wait() {
  ...
  const auto idle = m_pc_state.compare_exchange_strong(
      expected_state, pc_state::consumer, std::memory_order_acq_rel);
...
bool result_core_base::await(
    std::experimental::coroutine_handle<> caller_handle) noexcept {
  const auto state = m_pc_state.load(std::memory_order_acquire);
  if (state == pc_state::producer) {
...
bool result_core_base::await_via(
  ...
  auto handle_done_state = [this](await_context& await_ctx,
                                  bool force_rescheduling) -> bool {
    assert_done();
...
void result_core_base::when_all(
    std::shared_ptr<when_all_state_base> when_all_state) noexcept {
  const auto state = m_pc_state.load(std::memory_order_acquire);
  if (state == pc_state::producer) {
...
void result_core_base::try_rewind_consumer() noexcept {
  ...
  const auto consumer = this->m_pc_state.compare_exchange_strong(
      expected_consumer_state, pc_state::idle, std::memory_order_acq_rel);
