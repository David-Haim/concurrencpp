...
namespace mock_async_sql {
    ...
    struct connection_callback_base {
        ...
        virtual void on_connection(std::exception_ptr,
                                   std::shared_ptr<db_connection>) = 0;
    };

    struct query_callback_base {
        virtual ~query_callback_base() noexcept = default;

        virtual void on_query(std::exception_ptr,
                              std::shared_ptr<db_connection>,
    ...
    class db_connection : public std::enable_shared_from_this<db_connection> {
       ...
       public:
        db_connection(std::string db_url,
                      std::string username,
                      std::string password) {
            (void)db_url;
            (void)username;
        ...
        void query(std::string query_string,
