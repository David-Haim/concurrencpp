...
class mock_sql_runner {
 ...
 public:
  ...
  void mock_connection(std::shared_ptr<db_connection> connection_ptr,
                       std::unique_ptr<connection_callback_base> cb) {
    auto future = std::async(
        std::launch::async, [connection_ptr, cb = std::move(cb)]() mutable {
          std::this_thread::sleep_for(std::chrono::seconds(2));

          if (failed()) {
            auto error_ptr = std::make_exception_ptr(
                std::runtime_error("db_connection::connect - can't connect."));
  ...
  void mock_query_result(std::shared_ptr<db_connection> connection_ptr,
    ...
    auto future = std::async(
        std::launch::async, [connection_ptr, cb = std::move(cb)]() mutable {
          ...
          if (failed()) {
            auto error_ptr = std::make_exception_ptr(std::runtime_error(
                "db_connection::query - can't execute query."));
            return cb->on_query(error_ptr, nullptr, {});
          }

          std::vector<std::vector<std::any>> results;
          results.emplace_back(
              std::vector<std::any> {"Daniel", "Williams", 34});
...
void mock_async_sql::db_connection::connect(
    std::unique_ptr<connection_callback_base> connection_callback) {
  global_runner.mock_connection(shared_from_this(),
                                std::move(connection_callback));
}

void mock_async_sql::db_connection::query(
    std::string query_string,
    std::unique_ptr<query_callback_base> query_callback) {
  (void)query_string;
  global_runner.mock_query_result(shared_from_this(),
                                  std::move(query_callback));
}
