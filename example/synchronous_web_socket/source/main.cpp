/*
	...
	Since connecting and reading from a websocket may block for a long time (up to hours and days, if no update is available),
	and in an un-predictable manner, neither the background_executor or a concurrencpp::timer is suitable for
	the task.
	...
	There are merits in using concurrencpp::thread_executor over a raw std:thread :
		...
		executing tasks is decoupled away. if we want to change std::thread to let's say, a threadpool,
		we will have to rewrite these lines of code dealing with thread creation, task scheduling etc.
	...
	This example is only good for showing how to use a potentially long running/blocking tasks with a dedicated thread.
