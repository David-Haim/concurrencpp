...
URL=("https://github.com/llvm/llvm-project/releases/download/llvmorg-${VERSION}/" "-${VERSION}.src.tar.xz")
LLVM_URL="${URL[0]}llvm${URL[1]}"
LIBCXX_URL="${URL[0]}libcxx${URL[1]}"
LIBCXXABI_URL="${URL[0]}libcxxabi${URL[1]}"
...
if [[ $VERSION == *"3."* ]]; then
    ...
    make cxx -j $JOB_COUNT VERBOSE=1
    mkdir "${TARGET}/lib"
...
echo "::set-output name=include::${TARGET}/include"
echo "::set-output name=lib::${TARGET}/lib"
