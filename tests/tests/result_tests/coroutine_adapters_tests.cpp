...
namespace concurrencpp::tests {
	template<class type>
	result<type> sync_coro_val(object_observer::testing_stub stub);

	template<class type>
	result<type> sync_coro_err(object_observer::testing_stub stub);

	template<class type>
	result<type> async_coro_val(object_observer::testing_stub stub);

	template<class type>
	result<type> async_coro_err(object_observer::testing_stub stub);
...
namespace concurrencpp::tests::coroutines {
	std::shared_ptr<concurrencpp::runtime> get_coroutine_runtime() {
		static const std::shared_ptr<concurrencpp::runtime> s_runtime = concurrencpp::make_runtime();
		return s_runtime;
	}
}
...
template<class type>
result<type> concurrencpp::tests::sync_coro_val(object_observer::testing_stub stub) {
	auto _stub = std::move(stub);
	co_return result_factory<type>::get();
}

template<class type>
result<type> concurrencpp::tests::sync_coro_err(object_observer::testing_stub stub) {
	auto _stub = std::move(stub);
	co_return result_factory<type>::throw_ex();
}

template<class type>
result<type> concurrencpp::tests::async_coro_val(object_observer::testing_stub stub) {
	auto _stub = std::move(stub);
	auto res = coroutines::get_coroutine_runtime()->thread_pool_executor()->submit([]() -> decltype(auto) {
...
template<class type>
result<type> concurrencpp::tests::async_coro_err(object_observer::testing_stub stub) {
	auto _stub = std::move(stub);
	auto res = coroutines::get_coroutine_runtime()->thread_pool_executor()->submit([]() -> decltype(auto) {
		std::this_thread::sleep_for(std::chrono::milliseconds(15));
...
void concurrencpp::tests::test_non_ready_result_value_impl() {
	auto result_0 = coroutines::get_coroutine_runtime()->thread_pool_executor()->submit([]() -> decltype(auto) {
...
result<type> concurrencpp::tests::recursive_coro_value(const size_t cur_depth, const size_t max_depth) {
	...
	co_return co_await coroutines::get_coroutine_runtime()->thread_executor()->submit([]() -> decltype(auto) {
...
void concurrencpp::tests::test_coro_combo_value() {
	...
	auto int_coro = []() -> result<int> {
		co_return co_await coroutines::get_coroutine_runtime()->thread_pool_executor()->submit([] {
			std::this_thread::sleep_for(std::chrono::milliseconds(10));
	...
	auto str_ref_coro = [&, int_ref_result = std::move(int_ref_result)]() mutable->result<std::string&> {
		...
		assert_same(&int_res, &dummy_integer);
	...
	auto void_coro = [&, str_ref_result = std::move(str_ref_result)]() mutable->result<void> {
		...
		assert_same(&str_res, &dummy_string);
		assert_same(str_res, "12345678");
	};
