...
void concurrencpp::tests::test_result_promise_destructor_impl() {
	...
	assert_same(result.status(), result_status::EXCEPTION);
	assert_throws<concurrencpp::errors::broken_task>([&] { result.get(); });
}

void concurrencpp::tests::test_result_promise_RAII_impl() {
	object_observer observer;
	std::vector<result_promise<object_observer::testing_stub>> rps;
	std::vector<result<object_observer::testing_stub>> results;
	...
	results.clear();
	assert_same(observer.get_destruction_count(), 500'000);
}
...
void concurrencpp::tests::test_result_promise_get_result_impl() {
	...
	assert_true(static_cast<bool>(result));
	assert_same(result.status(), concurrencpp::result_status::IDLE);

	assert_throws<concurrencpp::errors::result_already_retrieved>([&] { rp.get_result(); });

	//empty rp should throw
	auto dummy = std::move(rp);
	assert_throws<concurrencpp::errors::empty_result_promise>([&] { rp.get_result(); });
}
...
void concurrencpp::tests::test_result_promise_set_value_impl(const arguments_tuple_type& tuple_args) {
	...
	//(by building an object in place) the future is un-ready
	{
		struct throws_on_construction {
			throws_on_construction(int, int) { throw std::exception(); }
		};
		...
		assert_true(static_cast<bool>(result));
		assert_true(static_cast<bool>(rp));
		assert_same(result.status(), result_status::IDLE);
	}
	...
	//setting result to an empty rp throws
	{
		...
		assert_throws<concurrencpp::errors::empty_result_promise>([
			rp = std::move(rp),
				set_rp,
				tuple_args]() mutable { set_rp(std::move(rp), tuple_args); });
	}
}
...
void concurrencpp::tests::test_rp_assignment_operator_impl_non_empty_to_non_empty() {
	...
	assert_same(result1.status(), result_status::EXCEPTION);
	assert_throws<concurrencpp::errors::broken_task>([&] { result1.get(); });

	assert_same(result2.status(), result_status::IDLE);
}
