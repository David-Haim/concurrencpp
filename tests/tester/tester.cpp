...
test_step::test_step(const char* step_name, std::function<void()> functor) :
	m_step_name(step_name),
	m_step(std::move(functor)) {}
...
void tester::add_step(const char* step_name, std::function<void()> functor) {
	m_steps.emplace_back(step_name, std::move(functor));
}
