  ...
  for (size_t i = 0; i < timer_count; i++) {
    ...
    testers
        .emplace_front(std::chrono::milliseconds(due_time),
                       std::chrono::milliseconds(frequency), timer_queue)
        .start_timer_test();
  }
...
void concurrencpp::tests::test_timer_destructor_dead_timer_queue() {
...

  {
    ...
    timer = timer_queue->make_timer(10 * 1000ms, 10 * 1000ms, executor,
                                    empty_callback);
  }
...
void concurrencpp::tests::test_timer_destructor_RAII() {
  ...
  for (size_t i = 0; i < timer_count; i++) {
    timers.emplace_back(timer_queue->make_timer(
        1'000ms, 1'000ms, inline_executor, observer.get_testing_stub()));
  }
...
void concurrencpp::tests::test_timer_cancel_before_due_time() {
  ...
  auto timer = timer_queue->make_timer(1'000ms, 500ms, wt_executor,
...
void concurrencpp::tests::test_timer_cancel_after_due_time_before_beat() {
  ...
  auto timer = timer_queue->make_timer(500ms, 500ms, wt_executor,
                                       observer.get_testing_stub());
...
void concurrencpp::tests::test_timer_cancel_after_due_time_after_beat() {
  ...
  auto timer = timer_queue->make_timer(1'000ms, 1'000ms, wt_executor,
                                       observer.get_testing_stub());
...
void concurrencpp::tests::test_timer_operator_bool() {
  ...
  auto timer_1 = timer_queue->make_timer(10 * 1000ms, 10 * 1000ms,
                                         inline_executor, empty_callback);
...
void concurrencpp::tests::
  ...
  auto timer0 = timer_queue->make_timer(10 * 1'000ms, 10 * 1'000ms, executor0,
                                        observer0.get_testing_stub());
  auto timer1 = timer_queue->make_timer(20 * 1'000ms, 20 * 1'000ms, executor1,
                                        observer1.get_testing_stub());
...
void concurrencpp::tests::test_timer_assignment_operator_empty_to_non_empty() {
  ...
  auto timer0 = timer_queue->make_timer(10 * 1'000ms, 10 * 1'000ms, executor,
                                        observer.get_testing_stub());
...
void concurrencpp::tests::test_timer_assignment_operator_non_empty_to_empty() {
  ...
  auto timer1 = timer_queue->make_timer(10 * 1'000ms, 10 * 1'000ms, executor,
                                        observer.get_testing_stub());
...
void concurrencpp::tests::test_timer_assignment_operator_assign_to_self() {
  ...
  auto timer = timer_queue->make_timer(1'000ms, 1'000ms, executor,
                                       observer.get_testing_stub());
