...
namespace concurrencpp::tests {
    ...
    template<class type>
    void test_shared_result_resolve_via_impl();
    void test_shared_result_resolve_via();
...
namespace concurrencpp::tests {
    ...
    struct test_resolve_via_ready_result<type, result_status::value, false, false> {
        result<void> operator()(std::shared_ptr<thread_executor> executor) {
            ...
            auto done_result = co_await sr.resolve_via(executor, false);
    ...
    struct test_resolve_via_ready_result<type, result_status::value, true, false> {
        result<void> operator()(std::shared_ptr<thread_executor> executor) {
            ...
            auto done_result = co_await sr.resolve_via(executor, true);
    ...
    struct test_resolve_via_ready_result<type, result_status::value, true, true> {
        result<void> operator()(std::shared_ptr<throwing_executor> executor) {
            ...
            try {
                co_await sr.resolve_via(executor, true);
    ...
    struct test_resolve_via_not_ready_result<type, result_status::value, false> {
       ...
       private:
        ...
        result<void> inner_task(std::shared_ptr<manual_executor> manual_executor, std::shared_ptr<thread_executor> thread_executor) {
            ...
            auto done_result = co_await sr.resolve_via(thread_executor, true);
    ...
    struct test_resolve_via_not_ready_result<type, result_status::value, true> {
       ...
       private:
        ...
        result<void> inner_task(std::shared_ptr<manual_executor> manual_executor, std::shared_ptr<throwing_executor> throwing_executor) {
            ...
            try {
                auto done_result = co_await sr.resolve_via(throwing_executor, true);
            } catch (const errors::broken_task&) {
                m_resuming_thread_id = thread::get_current_virtual_id();
                co_return;
            } catch (...) {
    ...
    struct test_resolve_via_not_ready_result<type, result_status::exception, true> {
       ...
       private:
        ...
        result<void> inner_task(std::shared_ptr<manual_executor> manual_executor, std::shared_ptr<throwing_executor> throwing_executor) {
            ...
            try {
                co_await sr.resolve_via(throwing_executor, true);
            } catch (const errors::broken_task&) {
                m_resuming_thread_id = thread::get_current_virtual_id();
                co_return;
            } catch (...) {
...
template<class type>
void concurrencpp::tests::test_shared_result_resolve_via_impl() {
    // empty result throws
    assert_throws_with_error_message<concurrencpp::errors::empty_result>(
        [] {
        ...
        concurrencpp::details::consts::k_shared_result_resolve_via_error_msg);
    ...
    test_resolve_via_not_ready_result<type, result_status::exception, true>()(manual_executor, throwing_executor, thread_executor).get();
}

void concurrencpp::tests::test_shared_result_resolve_via() {
    test_shared_result_resolve_via_impl<int>();
    test_shared_result_resolve_via_impl<std::string>();
    test_shared_result_resolve_via_impl<void>();
    test_shared_result_resolve_via_impl<int&>();
    test_shared_result_resolve_via_impl<std::string&>();
}

void concurrencpp::tests::test_shared_result_resolve_all() {
    tester tester("shared_result::resolve, shared_result::resolve_via test");

    tester.add_step("reslove", test_shared_result_resolve);
    tester.add_step("reslove_via", test_shared_result_resolve_via);
