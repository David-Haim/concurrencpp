...
struct costume_exception : public std::exception {
    ...
    costume_exception(intptr_t id) noexcept :
        id(id) {}
};
}  // namespace concurrencpp::tests
...
template<class type>
void test_ready_result_result(::concurrencpp::result<type> result) {
    test_ready_result_result<type>(std::move(result),
                                   result_factory<type>::get());
}
...
template<class type>
void test_ready_result_costume_exception(concurrencpp::result<type> result,
                                         const intptr_t id) {
    assert_true(static_cast<bool>(result));
