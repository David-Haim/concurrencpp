/*
	...
	db_connection::connect() and provides a callback that receives the asynchonous error or an open connection to 
	the data-base.

	The application then preceeds to call db_connection::query() and provides a callback that receives the asynchonous error or 
	the results in the form of std::vector< std::vector < std::any > > (a dynamic table).

	This example shows how to marshal asynchonous results and errors using concurrencpp::result_promise and 
	concurrencpp::result pair. this allows applications to avoid callback-style code flow and to use 
	coroutines instead.
...
result<std::shared_ptr<db_connection>> connect_async() {
	...
	public:
		...
		virtual void on_connection(std::exception_ptr error, std::shared_ptr<db_connection> connection) {
			if (error) {
...
void print_result(const std::vector<std::vector<std::any>>& results) {
	for (const auto line : results) {
		...
		auto first_name = std::any_cast<const char*>(line[0]);

		assert(line[1].has_value());
		assert(line[1].type() == typeid(const char*));

		auto last_name = std::any_cast<const char*>(line[1]);
