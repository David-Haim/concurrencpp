/*
	In this example, we'll use concurrencpp executors to process a file asynchronously,
	the application iterate over the file characters (we assume ASCII) and replaces a character with another.
	The application gets three parameters through the command line arguments:
	...
	Since standard file streams are blocking, we would like to execute file-io operations using
	the background_executor, who's job is to execute relatively short-blocking tasks (like file-io).
	...
	After content has been modified, it is ready to be re-written back to the file. we will again schedule a blocking
	write operation to the background_executor.
*/
...
concurrencpp::result<void> replace_chars_in_file(
	...
	//if we just await on the result returned by the background_executor, the execution resumes
	//inside the blocking-threadpool. it is important to resume execution in the cpu-threadpool
	//by calling result::await_via or result::resolve_via.
